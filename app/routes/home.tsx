...
export async function action({ request, context }: Route.ActionArgs) {
  ...
  if (intent === "signup") {
    ...
    if (!/^[a-z0-9_]{3,20}$/.test(handle)) {
      return data<ActionResult>({ error: "IDは3〜20文字の半角英数字と_で入力してください。", form: "signup" }, { status: 400 });
    }
    if (displayName.length < 1 || displayName.length > 30) {
      return data<ActionResult>({ error: "表示名は1〜30文字で入力してください。", form: "signup" }, { status: 400 });
    }
    if (password.length < 8 || password.length > 128) {
      return data<ActionResult>({ error: "パスワードは8〜128文字で入力してください。", form: "signup" }, { status: 400 });
    }
    ...
    try {
      await env.DB.prepare(
        `INSERT INTO users (id, handle, display_name, password_hash, password_salt)
         VALUES (?, ?, ?, ?, ?)`,
      ).bind(userId, handle, displayName, hash, salt).run();
    } catch {
  ...
  if (intent === "login") {
    ...
    if (!user?.password_hash || !user.password_salt || !(await verifyPassword(password, user.password_hash, user.password_salt))) {
  ...
  if (intent === "createPost") {
    ...
    if (!body || body.length > 280) return data<ActionResult>({ error: "投稿は1〜280文字で入力してください。" }, { status: 400 });
    await env.DB.prepare(
      "INSERT INTO posts (id, author_id, body, visibility) VALUES (?, ?, ?, 'public')",
    ).bind(crypto.randomUUID(), user.id, body).run();
...
function UserAvatar({ user, small = false }: { user: Pick<SessionUser, "displayName" | "handle">; small?: boolean }) {
  return <div className={`avatar ${avatarClass(user.handle)}${small ? " small" : ""}`}>{user.displayName.slice(0, 1)}</div>;
}
    ...
    <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <section className="auth-modal" role="dialog" aria-modal="true" aria-labelledby="auth-title">
        ...
        <Form method="post" className="auth-form">
          ...
          <label>パスワード<input name="password" type="password" required minLength={8} maxLength={128} autoComplete={mode === "login" ? "current-password" : "new-password"} /></label>
  ...
  if (!user) {
    return (
      <button className="login-prompt" onClick={onRequireLogin}>
        <LogIn size={18} /><span>ログインして、最初の投稿をしてみよう</span>
      </button>
    );
  }
  ...
  return (
    <fetcher.Form method="post" className="composer">
      ...
      <div className="composer-main">
        <textarea id="composer" name="body" value={draft} onChange={(event) => setDraft(event.target.value.slice(0, 280))} placeholder="いまどうしてる？" rows={2} />
        <div className="composer-footer">
          <div className="composer-tools"><button type="button" disabled title="画像投稿は次のアップデートで対応します"><ImageIcon size={19} /></button><button type="button">全員に公開</button></div>
          <div className="composer-submit">
            {draft.length > 0 && <span className={draft.length > 260 ? "limit near" : "limit"}>{280 - draft.length}</span>}
            <button type="submit" disabled={!draft.trim() || fetcher.state !== "idle"}>{fetcher.state === "idle" ? "投稿する" : "送信中"}</button>
          </div>
        </div>
...
function ReactionButton({ post, kind, user, onRequireLogin }: { post: TimelinePost; kind: "like" | "repost" | "bookmark"; user: SessionUser | null; onRequireLogin: () => void }) {
  const fetcher = useFetcher<ActionResult>();
...
function PostCard({ post, user, onRequireLogin }: { post: TimelinePost; user: SessionUser | null; onRequireLogin: () => void }) {
    ...
    <article className="post">
      ...
      <div className="post-content">
        ...
        <footer className="post-actions">
          <button onClick={user ? undefined : onRequireLogin} aria-label="返信"><span><MessageCircle size={18} /></span><small>{post.replies || ""}</small></button>
...
export default function HomePage({ loaderData, actionData }: Route.ComponentProps) {
  ...
  const openAuth = (mode: "login" | "signup") => { setDismissedAction(false); setAuthMode(mode); };
  ...
  return (
    <main className="app-shell">
      ...
      <section className="feed-column">
        <header className="feed-header"><div className="mobile-brand"><span className="brand-mark"><span /></span></div><div className="tabs" role="tablist">{(["おすすめ", "フォロー中"] as const).map((tab) => <button key={tab} role="tab" aria-selected={activeTab === tab} className={activeTab === tab ? "tab active" : "tab"} onClick={() => setActiveTab(tab)}>{tab}</button>)}</div><button className={`mobile-avatar avatar ${user ? avatarClass(user.handle) : "avatar-dark"}`} onClick={() => user ? undefined : requireLogin()}>{user?.displayName.slice(0, 1) ?? "?"}</button></header>
        <div className="topic-strip"><Sparkles size={15} /><span>いま話されていること</span><strong>みんなで決める最初の機能</strong><button>参加する</button></div>
        <Composer user={user} onRequireLogin={requireLogin} />
        <div className="feed-status"><span>{activeTab}の投稿</span><button>新しい順</button></div>
        <div className="posts" aria-live="polite">{visiblePosts.length === 0 ? <div className="empty-state"><Search size={28} /><strong>投稿が見つかりません</strong><span>{query ? "別の言葉で検索してみてください。" : "最初の投稿をしてみましょう。"}</span></div> : visiblePosts.map((post) => <PostCard key={post.id} post={post} user={user} onRequireLogin={requireLogin} />)}</div>
      </section>

      <aside className="rightbar"><div className="rightbar-inner">
        <label className="search-box"><Search size={18} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Commonsを検索" />{query && <button onClick={() => setQuery("")} aria-label="検索を消す">×</button>}</label>
        {!user && <section className="side-card join-card"><h2>Commonsをはじめよう</h2><p>登録は無料。メールアドレスは必要ありません。</p><button onClick={() => openAuth("signup")}>アカウントを作成</button><button className="secondary" onClick={() => openAuth("login")}>ログイン</button></section>}
        <section className="side-card project-card"><div className="eyebrow"><Code2 size={15} /> OPEN SOURCE</div><h2>このSNSを、一緒につくる。</h2><p>機能提案、デザイン、翻訳、コード。得意な方法で開発に参加できます。</p><a className="project-link" href="https://github.com/anitigravitylab-oss/commons-sns" target="_blank" rel="noreferrer">開発に参加する <span>→</span></a></section>
        <section className="side-card trends-card"><div className="card-title"><h2>いまの話題</h2><button><CircleEllipsis size={19} /></button></div>{[["コミュニティ", "最初にほしい機能"], ["開発", "#CommonsDev"], ["日本のトレンド", "小さなSNS"]].map(([kind, title]) => <button className="trend" key={title}><small>{kind}</small><strong>{title}</strong></button>)}</section>
        <footer className="legal-links"><a href="https://github.com/anitigravitylab-oss/commons-sns">ソースコード</a><a href="https://github.com/anitigravitylab-oss/commons-sns/blob/main/LICENSE">AGPL-3.0</a><span>© 2026 Commons</span></footer>
      </div></aside>
      <nav className="mobile-nav" aria-label="モバイルナビゲーション">{[Home, Search, Feather, Bell, UserRound].map((Icon, index) => <button key={index} className={index === 0 ? "active" : ""} onClick={index === 2 && !user ? requireLogin : undefined}><Icon size={23} /></button>)}</nav>
      {visibleAuthMode && <AuthModal mode={visibleAuthMode} error={actionData?.form === visibleAuthMode ? actionData.error : undefined} onClose={() => { setAuthMode(null); setDismissedAction(true); }} onChange={openAuth} />}
    </main>
  );
}
