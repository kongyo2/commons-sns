...
describe("PostBody", () => {
  ...
  it("URL のクエリの中や、日本語のメールアドレスもリンクにしない", () => {
    const html = renderInRouter(<PostBody body="https://example.com?q=@demo_aoi と 太郎@example.com" />);
    expect(html).not.toContain('href="/users/');
    expect(html).toContain("太郎@example.com");
  });
